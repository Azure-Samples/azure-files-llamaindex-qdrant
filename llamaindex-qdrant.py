"""RAG pipeline using LlamaIndex, Qdrant, and Azure Files.

Ingests documents from an Azure file share, converts files to LlamaIndex
Documents, indexes into Qdrant, and provides an interactive Q&A loop.
"""
...
def embed_and_index(nodes: list[TextNode]) -> VectorStoreIndex:
    """Embed text nodes via Azure OpenAI and upsert into a Qdrant collection.

    Args:
        nodes: A list of TextNode chunks to embed and index.

    Returns:
        A VectorStoreIndex connected to the populated Qdrant collection.
    """
...
def build_query_engine(index: VectorStoreIndex):
    """Create a LlamaIndex query engine over a Qdrant-backed index.
